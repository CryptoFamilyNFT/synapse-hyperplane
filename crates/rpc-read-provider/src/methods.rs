...
pub fn handle_get_account_info(
    ...
    let mut encoding = "base64";
    let mut commitment = "processed";
    ...
    if params.len() > 1 {
        if let Some(opts) = params[1].as_object() {
            ...
            if let Some(comm) = opts.get("commitment").and_then(|v| v.as_str()) {
                commitment = comm;
            }
            if let Some(wc) = opts.get("withContext").and_then(|v| v.as_bool()) {
                with_context = wc;
            }
        }
    }
